...
const CryptHistoryChart: React.FC<CryptHistoryChartProps> = ({
    id,
    interval,
    start,
    end,
    ...
    const { data, error, isLoading } =
        cryptAPI.useGetCryptocurrencyHistoryQuery({
            id,
            interval,
            start,
            end,
    ...
    const chartData = {
        labels:
            data?.data.map((entry) =>
                formatDateBasedOnInterval(new Date(entry.date), interval),
            ) || [],
        datasets: [
            {
                label: 'Price (USD)',
